    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 flex flex-col gap-4 hover:border-gray-700 transition-colors">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-gray-800/50 rounded-lg p-3">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {sideA.map((p) => (
              <span key={p.id} className="text-sm font-medium text-blue-300">
                {p.participantName}
              </span>
            ))}
          </div>
        ...
        <div className="bg-gray-800/50 rounded-lg p-3">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {sideB.map((p) => (
              <span key={p.id} className="text-sm font-medium text-rose-300">
                {p.participantName}
              </span>
            ))}
          </div>
      ...
      {(bet.eventDate || bet.resolutionDate) && (
        <div className="flex gap-4 text-xs text-gray-500">
          {bet.eventDate && <span>Event: {format(new Date(bet.eventDate), "MMM d, yyyy")}</span>}
          {bet.resolutionDate && (
      ...
      {bet.status === "RESOLVED" && bet.resolution && (
        <div className="bg-emerald-900/20 border border-emerald-500/20 rounded-lg p-3">
          <div className="text-xs text-emerald-400 font-semibold uppercase tracking-wide mb-1">
            Result
          </div>
          <div className="text-sm text-gray-300">
            {bet.resolution.winningSide === "PUSH" ? (
              <span className="font-medium text-amber-300">Push</span>
            ) : (
              <>
            ...
            {bet.resolution.moneyTransfers.length > 0 && (
              <span className="text-gray-400">
                ...
                {bet.resolution.moneyTransfers
                  .map((t) => `${t.fromName} -> ${t.toName} $${t.amount.toFixed(2)}`)
                  .join(", ")}
              </span>
            )}
          </div>
